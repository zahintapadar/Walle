...
final class WallpaperLibraryViewController: NSViewController, NSCollectionViewDataSource, NSCollectionViewDelegate {
    ...
    let topBar = NSStackView(views: [searchField, NSView(), aspectPopup, importButton, applyButton])
    ...
    @objc private func onImport() {
        // Import and auto-apply; then reload grid
    ...
    @objc private func onDoubleClickItem(_ sender: NSClickGestureRecognizer) {
        ...
        if let indexPath = collectionView.indexPathForItem(at: p) {
            let model = filtered[indexPath.item]
            coordinator.apply(wallpaper: model)
        }
    }
